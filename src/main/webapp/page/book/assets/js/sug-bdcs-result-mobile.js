...
var Sug = (function () {
    ...
    function _getElementsByClassName(className, elem) {
        ...
        for (var i = 0, l = children.length; i < l; i++) {
            if (classRegExp.test(children[i].className)) {
    ...
    Sug.prototype.bindSugListItemEvent = function () {
        ...
        _delegateEvent(this.options.sugListElem, 'touchstart', this.options.itemClassName, function (e) {
            stopEventBubble(e);
            var target = e.target || e.srcElement;
            self.clearCurrent();
            _addClass(this, self.options.currentClassName);
        ...
        _delegateEvent(this.options.sugListElem, 'click', this.options.itemClassName, function (e) {
            if (!$(e.target).hasClass('sug-use') && !$(e.target).parent().hasClass('sug-use')) {
                self.selectCurrent();
                self.submit();
            }
            else {
    ...
    Sug.prototype.setStyle = function () {
        ...
        var topHead = document.getElementById("head").offsetHeight + document.getElementById("head").offsetTop;
        var head_wd = document.getElementById("head").clientWidth;
        head_wd = head_wd - 2;
    ...
    Sug.prototype.getData = function (currentValue) {
        ...
        var limit = this.options.sugListItemLimit;
        var currentQuery = this.getCurrentValue();
        var queryLength = currentQuery.length;
        var sugQuery = '<span style="opacity:0.5;">' + currentQuery + '</span>';
        FlyJSONP.get({
            url: url,
            success: function (data) {
                ...
                var sugData;
                if (data.data.sug) {
                    sugData = data.data.sug;
                } else {
                    sugData = data.data;
                }
                var limitSug = sugData.length > limit ? limit : sugData.length;
                for (var i = 0; i < limitSug; i++) {
                    if (sugData[i].title.length >= queryLength) {
                        var sugFront = sugData[i].title.substring(0, queryLength);
                        if (sugFront == currentQuery) {
                            var sugRear = sugData[i].title.substring(queryLength);
                            sugData[i].title = sugQuery + sugRear;
                        }
                    }

                }
                self.cacheData(currentValue, sugData);
    ...
    Sug.prototype.hideIcon = function () {
        this.options.iconElem.style.display = 'none';
    };
    Sug.prototype.showIcon = function () {
        this.options.iconElem.style.display = 'block';
    };
    ...
    Sug.prototype.selectCurrent = function () {
        ...
        if (currentSelectedItem) {
            ...
            if (this.options.inputElem.value.indexOf("</span>") != -1) {
                var currentValue = this.getCurrentValue();
                this.options.inputElem.value = this.options.inputElem.value.substring(this.options.inputElem.value.indexOf(">") + 1, this.options.inputElem.value.indexOf("</span>")) + this.options.inputElem.value.substring(this.options.inputElem.value.indexOf("</span>") + 7);
            }
            this.hideSugList();
